...
class CrossAttention(nn.Module):
    def __init__(self, in_channels, emb_dim, out_dim, att_dropout=0.0, aropout=0.0):
        ...
        self.proj_out = nn.Conv2d(emb_dim, out_dim, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, context, pad_mask=None):
        ...
        x = self.proj_in(x)   # [batch_size, c, h, w] = in [1, 2048, 7, 7] out [1, 512, 7, 7]
        x = rearrange(x, 'b c h w -> b (h w) c')   # [batch_size, h*w, c] = [1, 49, 512]
        Q = self.Wq(x)  # [batch_size, h*w, emb_dim] = [1, 49, 512]
        K = self.Wk(context)  # [batch_szie, seq_len, emb_dim] = [1, 77, 512]
        V = self.Wv(context)
        ...
        if pad_mask is not None:
            # [batch_size, h*w, seq_len]
            pad_mask = pad_mask.repeat((1,196,1))
...
class TEC(nn.Module):
    """
    TEC: Text-Enhanced Contrastive model
    封装视觉主干、文本编码器、交叉注意力等为nn.Module类
    """
    def __init__(
        self,
        vision_model_name='senet154',
        num_classes=1000,
        loss='softmax',
        pretrained=True,
        ...
        super().__init__()
        # 视觉主干
        self.vision_encoder = eval(vision_model_name)(
            num_classes=num_classes, loss=loss, pretrained=pretrained
        ).to(device)
        # 交叉注意力
        ca_cfg = cross_attention_cfg or dict(in_channels=1024, emb_dim=512, out_dim=2048)
        self.cross_attention = CrossAttention(**ca_cfg).to(device)
        self.device = device

    def forward(self, image, text_feas, mask):
        """
        ...
        out0 = self.vision_encoder.layer0(x)
        out1 = self.vision_encoder.layer1(out0)
        out2 = self.vision_encoder.layer2(out1)
        out3 = self.vision_encoder.layer3(out2)
        out4 = self.vision_encoder.layer4(out3)

        # 交叉注意力
        ca_result, att = self.cross_attention(
            out3.type(torch.float32), text_feas.type(torch.float32), pad_mask=mask
        )
        ca3 = torch.nn.functional.adaptive_avg_pool2d(ca_result, (1,1))
        ca4 = torch.nn.functional.adaptive_avg_pool2d(out4, (1,1))
        fused = ca3 + ca4
